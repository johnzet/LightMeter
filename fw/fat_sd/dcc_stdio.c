...
// timeout modification by Martin Thomas, not available for __ARM_ARCH_7M__
#define DCC_STDIO_WITH_TIMEOUT  1
...
#if DCC_STDIO_WITH_TIMEOUT
void dbg_write(unsigned long dcc_data)
{
	unsigned long dcc_status;
	unsigned long countdown = 100000; // "magic" may have to be changed for other target/frequencies

	do {
		asm volatile("mrc p14, 0, %0, c0, c0" : "=r" (dcc_status));
		countdown--;
	} while ( ( dcc_status & 0x2 ) && ( countdown ) );
